...
- [] Move generation
    ...
    - [] Parallelism and concurrency (Performance Boost)
...
**Board representation is a mailbox array** - `Array Square (Maybe Piece)`, indexed by `newtype Square = Square Int` where 0 is a1 and 63 is h8.

This was chosen over bitboards for a first implementation due to simplicity. Coping via `//` is slow, intention to revisit once perft is passing. The flat `Int` index is deliberate, easier to transition to bitboards later.

**Parsing returns `Either String a`. ** which provides a soft landing for malformed FEN, one exception is `startingPosition` which calls `error` on a hardcoded starting position.

**Every record field is strict.** A lazy field in a `Position` holds a thunk that keeps the previous position alive; a few million of those during search could be a significant space problem. Strictness also helps to integrate
parallelism and concurrency.

**Move generation is piece-first, not side-first.** `knightMoves` generates moves for whatever knight sits on the given square, regardless of whose turn it is. Filtering by side to move happens at the caller. This keeps the same functions usable for attack detection which needs to ask about the opponent's pieces.
